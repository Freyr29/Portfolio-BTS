const items = await fetch('./interets/mesinteret.json').then(items => items.json());
...
function genererCadre(items) {
    for (let i = 0; i < items.length; i++) {
        ...
        const sectionFiches = document.querySelector(".mesinteret");
        ...
        const desc3Element = document.createElement("h6");
        desc3Element.innerText = items[i].comp5;
        pieceElement.appendChild(desc3Element);
